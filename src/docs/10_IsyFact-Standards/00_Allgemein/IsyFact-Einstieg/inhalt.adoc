...
Die <<IsyFactReferenzarchitektur>> definiert die Architektur einzelner Anwendungssysteme bis hin zu Anwendungslandschaften.
...
Innerhalb der Domänen werden wiederum Fachanwendungen geschnitten, um klar definierte Teilaspekte der Fachlichkeit umzusetzen.
Fachanwendungen weisen eine hohe fachliche Kohäsion auf und kommunizieren zur Erfüllung ihrer Aufgaben in der Regel mit weiteren Fachanwendungen.
...
Neben der Struktur geht sie auch auf die Kommunikation der Fachanwendungen untereinander sowie mit Anwendungssystemen anderer Anwendungslandschaften ein.

Die fachliche Architektur der Fachanwendungen wird in Spezifikationen dokumentiert.
...
Die Vorgaben dazu befinden sich mitsamt Vorgehen und Beispielen in der Vorlage für die Systemspezifikation (<<IsyFactSystemspezifikation>>).
...
Für die Erfassung von Anforderungslisten gibt es ebenfalls eine Vorlage (<<IsyFactVorlageAnforderungsliste>>).
...
Im Dokument <<AnleitungDatenflussdiagramme>> wird die Erstellung von Datenflussdiagrammen beschrieben und eine Leseanleitung dazu geliefert.
...
Im Dokument <<IsyFactReferenzarchitekturITSystem>> wird die softwaretechnische Architektur eingeführt und erläutert.
...
:desc-image-image008: Softwaretechnische Referenzarchitektur eines IT-Systems
...
Somit bedingt die Architektur einer Fachanwendung maßgeblich die Strukturierung des daraus entstehenden IT-Systems.
...
Eine beispielhafte Umsetzung eines IT-Systems zeigt die <<Vorlageanwendung>>.

Datenzugriff::
Der Datenzugriff erfolgt über JPA.
Das <<DetailkonzeptKomponenteDatenzugriff>> legt die genaue Verwendung von JPA fest und beschreibt die Prinzipien sowie das Vorgehen zur Persistierung von Datenmodellen.
...
Die Ergänzung durch technische Komponenten und weitere Vorgaben, z.B. zur Konfiguration der Komponenten, sind in <<DetailkonzeptKomponenteAnwendungskern>> festgelegt.
...
Das <<DetailkonzeptKomponenteBatch>> beschreibt die Vorgaben für Batches sowie den Aufbau der Batch-Komponente.
...
Das <<DetailkonzeptKomponenteService>> beschreibt die Vorgaben für Services sowie den Aufbau der Service-Komponente.
Die konkrete Umsetzung mit dem Service-Framework HTTP Invoker sind im <<KonzeptHttpInvoker>> sowie den <<NutzungsvorgabenHttpInvoker>> beschrieben.
...
Die Vorgaben dazu befinden sich mitsamt Vorgehen und Beispielen in der Vorlage für den Systementwurf (<<IsyFactSystementwurf>>).
...
Die wichtigsten Entscheidungen zu den zu nutzenden Produkten sind im <<ProduktKatalog>> festgelegt.
...
Die Grundlagen der TI-Architektur auf Ebene von Anwendungslandschaften und Anwendungssystemen ist in <<IsyFactReferenzarchitektur>> beschrieben.
Die zu verwendende Infrastruktur ist zum Teil durch den <<ProduktKatalog>> vorgegeben.

[[vorgaben-fuer-das-systemhandbuch]]
===== Vorgaben für das Systemhandbuch
Das Systemhandbuch ist gemäß der <<IsyFactVorlageSystemhandbuch>> zu erstellen und beschreibt die Installation, Konfiguration und den Betrieb eines IT-Systems.
...
Das <<KonzeptDatumZeit>> beschreibt die konzeptionellen Grundlagen der Verarbeitung von Datums- und Zeitwerten.
Die <<NutzungsvorgabenDatumZeit>> beschreiben alle Aspekte, die bei der Entwicklung einer Anwendung zu berücksichtigen sind.

[[administrative-überwachung-und-konfiguration]]
==== Administrative Überwachung und Konfiguration
Das Dokument <<KonzeptUeberwachungKonfiguration>> beschreibt, welche Arten von Konfiguration für eine Anwendung vorgesehen sind und wie diese umgesetzt werden sollen.
...
==== Behandlung von internationalen Sonderzeichen
Fachanwendungen müssen zum Teil mit Einträgen umgehen, die nicht den geläufigen Zeichenstandards und Codierungen unterliegen.
Im Dokument <<NutzungsvorgabenUmgangMitSonderzeichen>> werden Festlegungen getroffen, wie mit daraus resultierenden Problemstellungen umgegangen wird.
...
[[logging]]
==== Logging
Fachanwendungen sollten Logs einheitlich erstellen und auswerten können.
Das <<KonzeptLogging>> beschreibt die einheitliche Erstellung von Logs in Anwendungen sowie deren Auswertung auf fachlicher Ebene.
Die <<NutzungsvorgabenLogging>> beschreiben die technische Umsetzung des Loggings und die technischen Möglichkeiten der Auswertung.
...
Das <<KonzeptTaskScheduling>> beschreibt die konzeptionellen Grundlagen der Steuerung von Aufgaben.
Die <<NutzungsvorgabenTaskScheduling>> beschreiben alle Aspekte, die bei der Entwicklung einer Anwendung zu berücksichtigen sind, und alle bereits in der IsyFact definierten Aufgaben (wie z.B. das periodische Neuladen der Anwendungskonfiguration).
...
In Fachanwendungen müssen manchmal Polling-basierte Schnittstellen angesprochen werden.
...
Das Konzept findet sich in <<KonzeptPolling>>.
...
Die Nutzerdokumentation befindet sich unter <<NutzungsvorgabenPolling>>.
...
Diese werden in <<NutzungsvorgabenIsyUtil>> überblicksartig beschrieben.
...
Auf den https://www.bva.bund.de/DE/Das-BVA/Aufgaben/I/Informationstechnik/RegisterFactory/Produkt/produkt_rf_node.html#doc241990bodyText6[Webseiten der Register Factory] befindet sich eine Übersicht über vorhandene Bausteine, die in Zukunft potentiell als IsyFact-Erweiterung zur Verfügung gestellt werden.
...
Die <<NutzungsvorgabenBenutzerverwaltung>> zeigen, wie die Anwendungskomponente in eine Anwendung integriert und genutzt werden kann.

[[styleguide]]
==== Styleguide
Der Baustein Styleguide beschreibt die Gestaltung grafischer Oberflächen nach einheitlichen Gesichtspunkten.
Das zentrale Dokument, der <<Styleguide>>, enthält Vorgaben zu Aufbau und Gestaltung von Web-GUIs.
...
Die <<NutzungsvorgabenSessionManagement>> zeigen, wie mit Hilfe dieser Erweiterung Sessions in IsyFact Anwendungen verwaltet werden.
